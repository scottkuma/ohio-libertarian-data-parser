...
newDB = filePath+"Historical"+filePrefix+fileSep+timeComponent+fileSuffix
...
if len(csvfiles) < 88:
	print "Less than 88 county files - may be missing data files!"
	myInput = raw_input("Press Enter to continue...")
...
for onefile in csvfiles:
	...
	for row in reader:
		if len(row) == fieldsInLine:
			...
			#Are they a current Libertarian?
			if (row[PARTY_AFFILIATION] == "L"):
				isLVoter = True
			else:
				#Cycle through Election Fields, look for "L"s
				for fld in range(FIRST_ELECTION_FIELD, LAST_ELECTION_FIELD + 1):  #Must add 1 to get full range
					if row[fld] == "L":
						isLVoter = True
						break
			...
			if (isLVoter == True ) :
				numLibertarians += 1
				print numVotersParsed, numLibertarians, "::", countyname, countyCount, "::", row[LAST_NAME], row[FIRST_NAME], (row[PARTY_AFFILIATION] if row[PARTY_AFFILIATION] != "" else "-"),(row[lastPrimaryField] if row[lastPrimaryField] != "" else "-")
...
print("--- %s Historical Libertarians ---" % str(numLibertarians))
